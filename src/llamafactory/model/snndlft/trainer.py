...
class Trainer:
    ...
    def __init__(
        ...
        self.tb_writter = SummaryWriter(log_dir=args.logging_dir)
        if args is None:
                        ...
                        if isinstance(model, SNNPeftModel):
                            if getattr(model.base_model, "update_and_allocate", None) is not None:
                                ...
                                if layers_firing_rate is not None and total_batched_samples % self.finetuning_args.deltaT == 0:
                                        ...
                                        self.tb_writter.add_scalars("layers_firing_rate",
                                                                {f'layer{i}': value for i,value in enumerate(layers_firing_rate)}, self.state.global_step)
                                        self.tb_writter.add_scalars("layers_ipt",
                                                                {f'layer{i}': value for i,value in enumerate(layers_ipt)}, self.state.global_step)
                                        for key,value in rank_pattern.items():
                                            self.tb_writter.add_scalar("Ranknum/%s"%(key,), value.count(True), self.state.global_step) 
...
class AdaCustomSeq2SeqTrainer(Seq2SeqTrainer):
    r"""
